...
export function AssetStatisticsForm({
  ...
  onSubmit: (value: { assetCode: string; issuerId: string }) => void;
  pending: boolean;
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (issuerId.replace(/\s+/g, "").startsWith("S")) setIssuerId("");
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4" noValidate>
      <div className="grid gap-4 sm:grid-cols-[minmax(0,10rem)_1fr]">
        <Field
          label={copy.assetCodeLabel}
          hint={copy.assetCodeHint}
          error={errorFor("assetCode")}
        >
          {({ inputId, describedBy, invalid }) => (
            <Input
              ...
              placeholder={copy.assetCodePlaceholder}
              autoComplete="off"
              spellCheck={false}
              maxLength={12}
            />
        ...
        <Field
          ...
          hint={`${copy.issuerHint} Reading from ${label}.`}
          error={errorFor("issuerId")}
        >
          {({ inputId, describedBy, invalid }) => (
            <Input
              id={inputId}
